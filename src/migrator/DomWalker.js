...
const handleElement = ($element, $, onElement, onIssue, onOpLog) => {
    ...
    if (targetElements == undefined) {
        onIssue(new MigrationError(ConversionIssueCode.EMPTY_ELEMENT, issues.join(","), $element.toString()));
    } else {
        targetElements.forEach((convertedElement) => onElement(convertedElement));
        if (issues.length > 0) {
            onIssue(new MigrationError(ConversionIssueCode.OTHERS, issues.join(","), $element.toString()));
        }
    }
