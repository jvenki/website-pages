...
export class ErrorBoundary extends React.Component {
    ...
    render() {
        if (this.state.hasError) {
            return (<p style={{background: "red", color: "white", fontWeight: "bold"}}>Something went wrong while Rendering. The HTML has issues that React doesnt like even though Browser may be lenient</p>);
        }
...
export const renderAsHtml = (htmlString) => {
    try {
        return toHTMLText(htmlString);
    } catch (err) {
        return (<p style={{background: "red", color: "white", fontWeight: "bold"}}>Something went wrong while Rendering. The HTML has issues that React doesnt like even though Browser may be lenient</p>);
    }
};
